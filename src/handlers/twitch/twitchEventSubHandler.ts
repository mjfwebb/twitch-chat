  ...
  switch (data.eventType) {
    case "channel.chat.message": {
      const currentUserInfo =
        store.getState().userInformation[data.chatter_user_id];
      if (!currentUserInfo) {
        const userInfo = await fetchUserInformation(data.chatter_user_login);
        if (userInfo === null) {
          logger.warn(
            `User information not found for user ID: ${data.chatter_user_login}`
          );
        } else {
      ...
      switch (data.message_type) {
        case "text": {
          logger.info(
            `Received chat message from ${data.chatter_user_name}: ${data.message.text}`
          );
          break;
        }
      }
    ...
    case "channel.chat.message_delete": {
      ...
      logger.info(
        `Deleted chat message from ${data.target_user_name}: ${data.message_id}`
      );
      break;
    }
